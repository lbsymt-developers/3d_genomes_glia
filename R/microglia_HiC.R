library(HiCBricks)
mcool_path <- "../tmpTOP/data_raw/Human_cluster_mcool/MG_all_brain.txt_1kb_contacts.mcool"
...
Brick_export_to_sparse(Brick = My_BrickContainer,
                       out_file="../tmpTOP/data/brick_export_10000_MICROGLIA.tsv",
                       remove_file=TRUE,
                       resolution=10000,
                       sep="\t")
...
library(dplyr)
mt_contacts <- vroom::vroom("../tmpTOP/data/brick_export_10000_MICROGLIA.tsv")
mt_contacts <- na.omit(mt_contacts)
snps_contacts <- readr::read_csv("../tmpTOP/data/microglia_PROCESAMIENTO_SNPs.csv")
...
readr::write_csv(snps_data, "../tmpTOP/data/microglia_SNPs.csv")
